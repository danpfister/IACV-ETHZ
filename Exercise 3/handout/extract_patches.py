...
def extract_patches(img, p):
    ...
    patches = np.empty((H, W, n_channels*p**2))
    for h in range(H):
      for w in range(W):
        patches[h, w] = np.roll(img, shift=(1-h, 1-w), axis=(0, 1))[:p, :p].flatten()
    return patches
    '''
    H, W, n_channels = img.shape

    # Pad the image
    padded_img = np.pad(img, ((p // 2, p // 2), (p // 2, p // 2), (0, 0)), mode='wrap')

    # Calculate strides and shape for as_strided
    stride = padded_img.strides
    shape = (H, W, p, p, n_channels)

    # Create a view with patches using as_strided
    patches_view = np.lib.stride_tricks.as_strided(padded_img, shape=shape, strides=(stride[0], stride[1], stride[0], stride[1], stride[2]))

    # Reshape and return the patches
    return patches_view.transpose(0, 1, 4, 2, 3).reshape(H, W, n_channels * p ** 2)
